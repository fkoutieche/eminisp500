---
title: "E-mini S&P 500 Analysis"
author: "Fady Koutieche Aziz"
output: 
...
First we will create a dataframe filtering only data of the American Session

We create this dataframe filtering data only from the American Session to have
a clear picture of the price action, volume and other aspects of the market in
the hours where most of the trading will take place
...
mean(american$range)
```
Now we will create a dataframe that gets hour from the Hour column to do some 
Hourly analysis 
...
ggplot(hour_vol, aes(x=hour, y=avg_vol))+
  ...
  labs(title = "Volume",
       subtitle = "Average per Minute by Hour")
```
Since most daytraders will be active during the first 2-3 hours let's create a 
dataframe excluding the volume from 4:00 PM, which is exponentially higher compared
to other hours.
...
This chart will give us a better idea of how the volume evolves during the first
few hours from the market open
...
ggplot(trd_hours_vol, aes(x=hour, y=avg_vol))+
  ...
  labs(title = "Volume",
       subtitle= "Average per Minute by Hour")
```
It is very clear that the most amount of trades occur within the first 2-3 hours,
reaching it's lowest levels during 12 and 1 PM.

# Stats per Day

Let's first create a dataframe that will let us do some analysis by day of the week
We will start by extracting the Day name from the Date column:
...
Then, let's summarize Volume and define High of Day, Low of Day and make the operation
to determine the range of the day
...
```
On the E-Mini S&P 500 the price has moved on average 71 points per day in 
the last 5 months.
...
```
A total of 1,490,278 contracts have been traded on average for the last 5 months.
...
Now we will analyze variables such as Volume, Range by day of the week and
arrange the order by the days of the week
